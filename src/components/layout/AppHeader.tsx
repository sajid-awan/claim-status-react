...
export function AppHeader({ onOpenSidebar }: AppHeaderProps) {
  return (
    <header className="flex h-[60px] w-full min-w-0 shrink-0 items-center gap-2 overflow-hidden border-b border-black/[0.04] bg-white px-3 py-2.5 sm:gap-3 sm:px-5">
      <div className="flex min-w-0 items-center gap-2 xl:hidden">
        ...
        <img
          ...
          alt="SAVi Technology"
          className="h-7 w-auto max-w-[4.5rem] shrink object-contain object-left sm:h-8 sm:max-w-[5rem]"
        />
      </div>
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-3 md:gap-5">
        ...
        <div className="hidden min-w-0 flex-col gap-[7px] sm:flex md:w-[103px]">
          <p className="truncate text-base leading-4 text-ink">Hassan, Sajid</p>
          <p className="truncate text-sm leading-[14px] text-ink/40">User</p>
        </div>
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-500 text-sm leading-6 text-white sm:h-10 sm:w-10 sm:text-base">
          SH
        </div>
      </div>
    </header>
